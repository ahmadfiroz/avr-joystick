...
from __future__ import division  # Import this if using python interpreter v 2.7
from visual import *
...
class Robot:
    ...
    def left(self, angle):
        ...
        if self._speed == Inf:
            ...
            self._frame.rotate(angle=radians(mod(psi - self.heading() + 180, 360) - 180), axis=(0,0,1))
    ...
    def save_camera(self):
        """Save current camera view of this robot.
        """
        self._camera_visible = self._frame.visible
        self._camera_autoscale = scene.autoscale
        self._camera_forward = vector(scene.forward)
        self._camera_up = vector(scene.up)
        self._camera_center = vector(scene.center)

    def restore_camera(self):
        """Restore previously saved camera view of this robot.
        """
        self._frame.visible = self._camera_visible
        scene.autoscale = self._camera_autoscale
        scene.forward = self._camera_forward
        scene.up = self._camera_up
        scene.center = self._camera_center

    def look(self):
        """Change camera view to look from this robot.

        The camera does not automatically move and turn with the robot.
        Call look() after each motion command to move with the robot.
        Use save_camera() and restore_camera() to restore the "top-down"
        view of the robot.
        """
        self.hide()
        scene.autoscale = False
        scene.forward = vector(self._frame.axis)
        scene.up = (0, 0, 1)
        scene.center = self._frame.pos + (0, 0, 8) + scene.forward * mag(
...
def maze(pos=(0,0), rows=8, columns=8, cell_size=30):
    ...
    # "Refresh" display to make the temporary autocenter stick.
    rate(10)
    rate(10)
